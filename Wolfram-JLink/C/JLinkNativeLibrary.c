...
static void setErrMsgElement(JNIEnv *env, const char *errMsgString, jobjectArray errMsgOut);
...
JNIEXPORT jlong MLFUNC(MLOpenString), jstring cmdLine, jobjectArray errMsgOut) {
	...
	if (utfString == NULL) {
		setErrMsgElement(env, "No command line", errMsgOut);
		...
		DEBUGSTR1(utfString)
		if (!initEnvironment()) {
			setErrMsgElement(env, "Failed to initialize the environment", errMsgOut);
			return JLONG_FROM_PTR(0);
		}
		...
		if (link != NULL) {
			...
			setErrMsgElement(env, MLErrorString(gMLEnv, err), errMsgOut);
		} /* else no custom error message, so if we continue to see a canonical "1004: Link failed to open", this is why */
...
static void setErrMsgElement(JNIEnv *env, const char *errMsgString, jobjectArray errMsgOut) {
	jstring errMsg = (*env)->NewStringUTF(env, errMsgString);
	(*env)->SetObjectArrayElement(env, errMsgOut, 0, errMsg);
}
