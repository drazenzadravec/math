...
public class ObjectHandler {
    ...
    protected JLinkClassLoader jlinkLoader = JLinkClassLoader.createClassLoader();
...
class ClassRecord {
	...
	ClassRecord(String name, ClassLoader loader, String vmName) throws ClassNotFoundException, SecurityException {
		...
		// We sort the methods and ctors below to ensure that they always get the same ordering. The index into these arrays becomes part of the
		// WL defs created, so the indices need to be the same from one Java session to the next, and for multiple JVMs running in the same WL session
		// (see https://jira.wolfram.com/jira/browse/ECE-391).
        ctors = cls.getConstructors(); // Publics only
        Arrays.sort(ctors, new Comparator<Constructor>() {
            @Override
            public int compare(Constructor c1, Constructor c2) {
                int argcountCompare = Integer.compare(c1.getParameterCount(), c2.getParameterCount());
                if (argcountCompare == 0) {
                    // Same number of args; sort by parameter names
        ...
        Arrays.sort(methods, new Comparator<Method>() {
            ...
            public int compare(Method m1, Method m2) {
                ...
                if (nameCompare == 0) {
                    // Same method name; sort by parameter types
        ...
        if (!Modifier.isPublic(m)) {
            try {
                // This next line helps eliminate the "An illegal reflective access operation has occurred" warning in Java 9 and later.
                // We dynamically open up the class's module to the unnamed module (J/Link is not in a module). See 396512.
                // It is not clear how effective this is, because we still need to use --add-opens arguments when launching J/Link to eliminate
                // the error in most other classes where it appears.
                ...
                cls.getModule().addOpens(cls.getPackageName(), ObjectHandler.class.getModule());
            } catch (Throwable t) {
                // Catch whatever junk the addOpens() call can throw, now or in the future.
            }
            try {
                ...
                System.err.println("Warning: the non-public class " + name + " was loaded by J/Link, and the attempt " +
                                            ...
                                            "when they are used from the Wolfram Language. See the documentation for the class " +
