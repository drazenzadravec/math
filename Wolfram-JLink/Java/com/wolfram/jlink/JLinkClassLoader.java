...
public class JLinkClassLoader extends ClassLoader {
    ...
    public Enumeration<URL> getResources(String name) throws IOException { return helper.getResources(name); }
   ...
   // don't want to override getPackage because it's not *this* classloader that has defined it
     ...
     * In Java 9 and later, this will return an instance of JLinkClassLoader9, a subclass that has special capabilities to support
     * ExternalEvaluate functionality in WL.
     * 
     * @return the newly-created JLinkClassLoader
     */
    public static JLinkClassLoader createClassLoader() {
        if (Utils.getJavaVersion() >= 9) {
            try {
                // We create a JLinkClassLoader9 via reflection so as not to trigger errors in a Java 1.8 VM.
                Class<?> jlinkClassLoader9  = Class.forName("com.wolfram.jlink.JLinkClassLoader9");
                return (JLinkClassLoader) jlinkClassLoader9.getDeclaredConstructor().newInstance();
            } catch (Exception e) {
                // This should never happen--the class is part of JLink.jar. Here we are just making the compiler happy.
                return null;
            }
       } else {
            return new JLinkClassLoader();
        }
    }
    ...
    public synchronized void addLocations(String[] locations, boolean searchForJars, boolean prepend) {
		...
		List<URL> urlsToAdd = new ArrayList<URL>(locations.length);
	...
	protected void addAll(List<URL> urlsToAdd, boolean prepend) {
	    ...
	    if (prepend) {
	        if (urlsToAdd.size() > 0) {
	            ...
	            List<URL> newURLSet = new ArrayList<URL>(existingLocs.length + urlsToAdd.size());
	        ...
	        Iterator<URL> iter = urlsToAdd.iterator();
    		while (iter.hasNext())
